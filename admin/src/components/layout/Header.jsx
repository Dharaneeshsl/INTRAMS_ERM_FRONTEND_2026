...
export default function Header() {
  ...
  return (
    <header className="fixed left-[260px] right-0 top-0 z-30 h-[60px] border-b border-[#252525] bg-[#000000]/95 backdrop-blur-md">
      <div className="flex h-full items-center justify-between gap-4 px-6 lg:px-8">
        <div className="flex min-w-0 items-center gap-3">
          <div className="min-w-0 truncate font-heading text-[15px] font-bold uppercase tracking-[0.2em] text-[#FFFFFF]">
            INTRAMS ERM ADMIN
          </div>
          {user?.username && (
            <span className="hidden min-w-0 truncate text-[12px] text-[#A0A0A0] uppercase font-bold sm:inline-block">
              · {user.username}
            </span>
          )}
        </div>
